...
(function($,undefined) {
    ...
    var toolGroups = {
        basic: [ "bold", "italic", "underline","myformat" ],
...
var InlineFormatter = Class.extend({
    ...
    apply: function (nodes) {
        ...
        for (i = 0, l = nodes.length; i < l; i++) {
			...
			var $_editor = $("#editor").data("kendoEditor");
			var $_spanCount = $($_editor.body).find('label').length + 1;
			if( $(formatNode).is('label') ) {
				$(formatNode).attr('id', 'myspan' + $_spanCount);
...
registerFormat("myformat", [ { tags: ["label","b"] }, { tags: ["span"], attr: { id: "myformat" } } ]);
registerTool("myformat", new InlineFormatTool({ key: "Z", ctrl: true, format: formats.myformat, template: new ToolTemplate({template: EditorUtils.buttonTemplate, title: "Myformat"}) }));
...
(function($,undefined) {
    ...
    var Toolbar = Widget.extend({
        ...
        groups: {
            basic: ["bold", "italic", "underline", "strikethrough", "myformat" ],
